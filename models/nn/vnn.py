...
class ConvFrameMaskDecoderProgressMonitor(nn.Module):
    ...
    def forward(self, enc_goal, enc_instr, frames, gold=None, max_decode=150, state_0_goal=None, state_0_instr=None):
        ...
        progresses = []
        for t in range(max_t):
            action_t, mask_t, state_t_goal, state_t_instr, attn_score_t_goal, attn_score_t_instr, subgoal_t, progress_t = self.step(enc_goal, enc_instr, frames[:, t], e_t, state_t_goal, state_t_instr)
            masks.append(mask_t)
            actions.append(action_t)
